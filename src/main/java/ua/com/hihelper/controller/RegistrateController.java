...
public class RegistrateController {
	 ...
	 * String registration(Model model){ model.addAttribute("user", new User());
	 * return "registration"; }
	 * 
	 * @RequestMapping(value="/registration", method=RequestMethod.POST) public
	 * String registration(@ModelAttribute User user){ userservice.save(user);
	 * mailservice.sendMessage("registration", user.getEmail(),
	 * "You are registered in hihelper."); return "redirect:/"; }
	 */
